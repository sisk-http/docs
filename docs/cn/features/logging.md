...
您可以配置 Sisk 自动写入访问和错误日志。可以定义日志轮换、扩展和频率。

[LogStream](/api/Sisk.Core.Http.LogStream) 类提供了一种异步写入日志和保持在可等待写入队列中的方式。`LogStream` 类实现了 `IAsyncDisposable`，确保在流关闭之前写入所有待处理日志。

在本文中，我们将向您展示如何为您的应用程序配置日志记录。

## 基于文件的访问日志

日志写入文件时，会打开文件，写入行文本，然后在每行写入后关闭文件。这种方法是为了保持日志的写入响应性。
...
上面的代码将写入所有传入的请求到 `logs/access.log` 文件。请注意，如果文件不存在，会自动创建文件，但是文件夹不会自动创建。您不需要创建 `logs/` 目录，因为 `LogStream` 类会自动创建它。
...
对于流式日志记录中的每个消息，会调用 `TextWriter.Flush()` 方法。
...
| %lms   | 耗时（毫秒）                                                   | 120                                  |
| %ls    | 执行状态                                                                | Executed                |
| %{header-name}    | 代表请求的 `header-name` 标头。                                                                | `Mozilla/5.0 (platform; rv:gecko [...]`                |
| %{:header-name}    | 代表响应的 `header-name` 标头。 | `application/json` |

您还可以使用 `HttpServerConfiguration.DefaultAccessLogFormat` 来使用默认的访问日志格式。

## 日志轮换

您可以配置 HTTP 服务器在日志文件达到一定大小时将其轮换到压缩的 .gz 文件。大小是由您定义的阈值周期性检查的。
...
上面的代码将每 6 小时检查一次 LogStream 文件是否达到 64MB 限制。如果达到限制，文件将被压缩到 .gz 文件，然后 `access.log` 将被清空。
...
您的应用程序可以有零个或多个 LogStreams，没有限制。因此，可以将应用程序的日志定向到其他文件，而不是默认的 AccessLog 或 ErrorLog。
...
class JsonMessageHandler : HttpServerHandler
{
    protected override void OnHttpRequestOpen(HttpRequest request)
    {
        if (request.Method != HttpMethod.Get && request.Headers["Content-Type"]?.Contains("json", StringComparison.InvariantCultureIgnoreCase) == true)
        {
            ...
            // 下面一行读取内容并将其存储在请求中。
            //
            // 如果在请求操作中不读取内容，内容可能会在发送响应给客户端后被 GC 收集，因此在响应关闭后可能无法访问内容。
            //
            _ = request.RawBody;

            // 在上下文中添加提示，以表明此请求具有 JSON 正文。
