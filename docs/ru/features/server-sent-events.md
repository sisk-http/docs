...
Эта функция имеет некоторые ограничения, наложенные браузерами, такие как отправка только текстовых сообщений и невозможность постоянного закрытия соединения. Соединение, закрытое на стороне сервера, будет иметь клиент, который периодически попытается переподключиться каждые 5 секунд (3 для некоторых браузеров).

Эти соединения полезны для отправки событий от сервера к клиенту без необходимости запроса информации клиентом каждый раз.
...
Вызов метода [HttpRequest.GetEventSource()](/api/Sisk.Core.Http.HttpRequest.GetEventSource) помещает запрос в состояние ожидания, пока не будет создан экземпляр SSE.
...
В вышеуказанном коде мы создаем соединение SSE и отправляем сообщение "Hello, world", затем мы закрываем соединение SSE с серверной стороны.

> [!NOTE]
> Когда соединение закрывается на стороне сервера, по умолчанию клиент попытается подключиться снова, и соединение будет перезапущено, выполняя метод снова, бесконечно.
>
> Обычно передается сообщение о завершении от сервера всякий раз, когда соединение закрывается на стороне сервера, чтобы предотвратить повторное подключение клиента.
...
Соединения обычно завершаются, когда сервер больше не может отправлять сообщения из-за возможного отключения клиента. После этого соединение автоматически завершается, и экземпляр класса удаляется.
...
Вышеуказанный метод создаст соединение, обработает его и будет ожидать отключения или ошибки.
...
И вышеуказанный фрагмент кода попытается найти только что созданное соединение, и если оно существует, оно отправит сообщение ему.
...
{
    e.Send("Транслирование всем источникам событий, которые начинаются с 'my-connection-'");
}
```
