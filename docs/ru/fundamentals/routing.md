...
Когда сервер получает запрос, Sisk ищет маршрут, который удовлетворяет выражению пути, полученному в запросе. Это выражение всегда тестируется между маршрутом и путем запроса, без учета строки запроса.
...
Sisk проверяет возможность коллизий маршрутов, чтобы избежать этих проблем. Когда определяются маршруты, Sisk будет искать возможные маршруты, которые могут столкнуться с определяемым маршрутом. Этот тест включает проверку пути и метода, который маршрут должен принять.
...
mainRouter += Route.Get("/image.png", (request) =>
{
    ...
    {
        // поток StreamContent будет disposed после отправки
        // ответа.
...
Каждый путь, полученный сервером, нормализуется перед выполнением теста шаблона пути, следующими правилами:
...
> [!NOTE]
> Пути игнорируют завершающий `/` как в запросе, так и в пути маршрута, то есть если вы попытаетесь получить доступ к маршруту, определённому как `/index/page`, вы сможете получить доступ, используя `/index/page/` тоже.
>
...
Строка ниже определит оба метода `Index` и `Hello` класса `MyController` как маршруты, поскольку они помечены как маршруты, и предоставлен экземпляр класса, а не его тип. Если бы вместо этого был предоставлен тип, были бы определены только статические методы.
...
public abstract class ControllerBase
{
    ...
    // строка ниже, когда она вызывается, получает базу данных из текущей HTTP-сессии,
...
Более подробную информацию о текущем контексте и внедрении зависимостей можно найти в [уроке по внедрению зависимостей](/docs/ru/features/instancing).
...
Обратные вызовы маршрутов могут выбрасывать ошибки во время выполнения сервера. Если они не обрабатываются правильно, общая работа HTTP-сервера может быть прервана. Маршрутизатор имеет обратный вызов для случая, когда обратный вызов маршрута не удался и предотвращает прерывание службы.
